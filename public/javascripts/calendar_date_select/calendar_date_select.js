...
CalendarDateSelect.prototype = {
  ...
  positionCalendarDiv: function() {
    ...
    var w_top = window.f_scrollTop(), w_height = window.f_height(), w_width = window.f_width();;
    var e_dim = $(this.options.get("popup_by")).cumulativeOffset(), e_top = e_dim[1], e_left = e_dim[0], e_height = $(this.options.get("popup_by")).getDimensions().height, e_bottom = e_top + e_height;
    
    var e_left_new = e_left;
    var width_diff = (w_width - 20) - (e_left + c_width);
    if (width_diff < 0) {
      e_left_new = e_left + width_diff;
    }

    if ( (( e_bottom + c_height ) > (w_top + w_height)) && ( e_bottom - c_height > w_top )) above = true;
    //var left_px = e_left.toString() + "px", top_px = (above ? (e_top - c_height ) : ( e_top + e_height )).toString() + "px";
    var left_px = e_left_new.toString() + "px", top_px = (above ? (e_top - c_height ) : ( e_top + e_height )).toString() + "px";
