...
(function() {
	...
	tinymce.create('tinymce.plugins.PrefMediaPlugin', {
		init : function(ed, url) {
			...
			ed.addCommand('mceMedia', function() {
				...
				ed.windowManager.open({
					...
					height : 580 + parseInt(ed.getLang('media.delta_height', 0)),
					inline : 1
				}, {
					plugin_url : url,
					data : data
			...
			// Register buttons
			ed.addButton('prefmedia', {title : 'media.desc', cmd : 'mceMedia', image: '/_common/tiny_mce/plugins/prefmedia/img/ic_video.gif'});
		...
		dataToImg : function(data, force_absolute) {
			...
			img.width = data.width = normalizeSize(data.width || (data.type == 'audio' ? "300" : "480"));
			img.height = data.height = normalizeSize(data.height || (data.type == 'audio' ? "32" : "360"));
		...
		objectToImg : function(node) {
			...
			img.attr({
				...
				style : style,
				width : width || (node.name == 'audio' ? "300" : "480"),
				height : height || (node.name == 'audio' ? "32" : "360"),
				hspace : hspace,
				vspace : vspace,
				align : align,
				bgcolor : bgcolor,
	...
	// Register plugin
	tinymce.PluginManager.add('prefmedia', tinymce.plugins.PrefMediaPlugin);
})();
