(function() {
	...
	window.Media = {
		...
		insert : function() {
			...
			editor.selection.setNode(editor.plugins.prefmedia.dataToImg(this.data));
			tinyMCEPopup.close();
		},

		preview : function() {
			get('prev').innerHTML = this.editor.plugins.prefmedia.dataToHtml(this.data, true);
		},

		moveStates : function(to_form, field) {
			var data = this.data, editor = this.editor,
				mediaPlugin = editor.plugins.prefmedia, ext, src, typeInfo, defaultStates, src;
			...
			if (to_form) {
				...
				// Set default size
                setVal('width', data.width || (data.type == 'audio' ? 300 : 480));
                setVal('height', data.height || (data.type == 'audio' ? 32 : 360));
			}
		},
		...
		formToData : function(field) {
			...
			if (field == 'source') {
				...
				setVal('source', this.editor.plugins.prefmedia.dataToHtml(this.data));
				this.panel = 'source';
			} else {
				if (this.panel == 'source') {
					this.data = clone(this.editor.plugins.prefmedia.htmlToData(getVal('source')));
					this.dataToForm();
					this.panel = '';
				}
		...
		beforeResize : function() {
            this.width = parseInt(getVal('width') || (this.data.type == 'audio' ? "300" : "480"), 10);
            this.height = parseInt(getVal('height') || (this.data.type == 'audio' ? "32" : "360"), 10);
		},
		...
		changeSize : function(type) {
			...
			if (get('constrain').checked) {
                width = parseInt(getVal('width') || (this.data.type == 'audio' ? "300" : "480"), 10);
                height = parseInt(getVal('height') || (this.data.type == 'audio' ? "32" : "360"), 10);
