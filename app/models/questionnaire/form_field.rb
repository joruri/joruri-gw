...
class Questionnaire::FormField < Gw::Database
  ...
  def update_field_name
    ...
    for fld in items
      ...
      Questionnaire::FormField.update_all("field_name='#{field_name}',auto_number=#{auto_number}", "id=#{fld.id}")
  ...
  def update_group_field_name
    ...
    for fld in items
      ...
      Questionnaire::FormField.update_all("group_field='#{field_name}'", "id=#{fld.id}")
    end unless items.blank?
  end
  #----------------------------------------------------------------------------
  ...
  def create_form_fields
    ...
    Questionnaire::Base.update_all("form_body='#{body_string}'", "id=#{self.parent_id}")
  end
