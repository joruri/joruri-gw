...
class Gwfaq::Control < Gw::Database
  ...
  def save_admingrps
    ...
    unless self.admingrps_json.blank?
      Gwfaq::Adm.destroy_all("title_id=#{self.id}")
      groups = JsonParser.new.parse(self.admingrps_json)
      @dsp_admin_name = ''
      groups.each do |group|
        item_grp = Gwfaq::Adm.new()
    ...
    unless self.dsp_admin_name == @dsp_admin_name
      args = ["UPDATE gwfaq_controls SET dsp_admin_name = ? WHERE id =?", @dsp_admin_name, self.id]
  ...
  def save_editors
    unless self.editors_json.blank?
      Gwfaq::Role.destroy_all("title_id=#{self.id} and role_code = 'w'")
      groups = JsonParser.new.parse(self.editors_json)
      groups.each do |group|
        unless group[1].blank?
          item_grp = Gwfaq::Role.new()
          item_grp.title_id = self.id
  ...
  def save_readers
    unless self.readers_json.blank?
      Gwfaq::Role.destroy_all("title_id=#{self.id} and role_code = 'r'")
  ...
  def save_readers_add
    unless self.editors_json.blank?
      ...
      item = Gwfaq::Role.find(:all, :conditions => "title_id=#{self.id} and role_code = 'r' and group_code = '0'")
      if item.length == 0
        ...
        groups.each do |group|
          unless group[1].blank?
            ...
            item_grp = Gwfaq::Role.find(:all, :conditions => "title_id=#{self.id} and role_code = 'r' and group_id = #{group[1]}")
  ...
  def create_faq_system_database
  ...
  def create_table_categories
    strsql = "CREATE TABLE IF NOT EXISTS `#{self.dbname}`.`gwfaq_categories` ("
  ...
  def create_table_docs
    strsql = "CREATE TABLE IF NOT EXISTS `#{self.dbname}`.`gwfaq_docs` ("
    strsql += "`id` int(11) NOT NULL auto_increment,"
    strsql += "`unid` int(11) default NULL,"
  ...
  def create_table_db_files
    strsql = "CREATE TABLE IF NOT EXISTS `#{self.dbname}`.`gwfaq_db_files` ("
    strsql += "`id` int(11) NOT NULL auto_increment,"
    strsql += "`title_id` int(11) default NULL,"
    strsql += "`parent_id` int(11) default NULL,"
  ...
  def create_table_files
    strsql = "CREATE TABLE IF NOT EXISTS `#{self.dbname}`.`gwfaq_files` ("
    strsql += "`id` int(11) NOT NULL auto_increment,"
    strsql += "`unid` int(11) default NULL,"
    strsql += "`content_id` int(11) default NULL,"
    strsql += "`state` text,"
  ...
  def cretae_table_images
    strsql = "CREATE TABLE IF NOT EXISTS `#{self.dbname}`.`gwfaq_images` ("
    strsql += "`id` int(11) NOT NULL auto_increment,"
    strsql += "`unid` int(11) default NULL,"
    strsql += "`content_id` int(11) default NULL,"
    strsql += "`state` text,"
  ...
  def create_table_recognizers
    strsql = "CREATE TABLE IF NOT EXISTS `#{self.dbname}`.`gwfaq_recognizers` ("
    strsql += "`id` int(11) NOT NULL auto_increment,"
    strsql += "`unid` int(11) default NULL,"
    strsql += "`updated_at` datetime default NULL,"
    strsql += "`created_at` datetime default NULL,"
