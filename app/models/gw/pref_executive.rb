class Gw::PrefExecutive < Gw::Database
  ...
  def self.get_members(options={})
    rs = self.where(:is_other_view => 1)
    if options[:u_lname]
      rs = rs.where.not(:u_lname => nil)
    else
      rs = rs.where(:u_lname => nil)
    end
    rs
  end
  ...
  def toggle_state(new_state = nil)
    ...
    Gw::PrefDirector.where(:uid => uid).update_all(:state => new_state)
    self
  end
  ...
  def self.is_admin?(user = Core.user)
    user.has_role?('gw_pref_executive/admin', '_admin/admin')
  end
