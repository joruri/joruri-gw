...
class Gw::PrefExecutive < Gw::Database
  ...
  def save_with_rels(params, mode, options={})
    ...
    if self.errors.size > 0
      ...
      old_users = Gw::PrefExecutive.find(:all, :conditions=>"state != 'deleted' AND deleted_at IS NULL")
      old_users.each_with_index{|old_user, x|
        ...
        users.each_with_index{|user, y|
          if old_user.uid.to_i == user[1].to_i and old_user.title == user[3]
            ...
            old_user.u_lname    = user[3] if u_lname_up == true
      ...
      users.each_with_index{|user, y|
        ...
        u_lname_up = true if user[3] =~ /副知事/
        u_lname_up = true if user[3] =~ /政策監/
        new_user.u_lname    = user[3] if u_lname_up == true
  ...
  def self.truncate_table
    ...
    truncate_query = "TRUNCATE TABLE `gw_pref_executives` ;"
