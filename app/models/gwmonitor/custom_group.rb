class Gwmonitor::CustomGroup < Gw::Database
  ...
  def check_readers
    if self.reader_groups_json.blank?
      ...
      objects = JSON.parse(self.reader_groups_json)
      if objects.count == 0
        errors.add :reader_groups_json, "を設定してください。"
      end
    end
  ...
  def self.custom_select
    list = []
    Gwmonitor::CustomGroup.where(:state => 'enabled', :owner_uid => Core.user.id).order('sort_no,id').each do |dep|
  ...
  def self.first_group_id
    item = Gwmonitor::CustomGroup.where(:state => 'enabled', :owner_uid => Core.user.id).order('sort_no,id').first
    ret = 0
  ...
  def self.get_user_select(gid)
    item = Gwmonitor::CustomGroup.where(:id => gid).first
    selects = []
    return selects if item.blank?
    return selects if item.reader_groups_json.blank?

    users = JSON.parse(item.reader_groups_json)
    users.each do |user|
