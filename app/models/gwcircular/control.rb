...
#  belongs_to :status, :foreign_key => :state, :class_name => 'Sys::Base::Status'
  ...
  def save_admingrps
    ...
    unless self.admingrps_json.blank?
      Gwcircular::Adm.destroy_all("title_id=#{self.id}")
  ...
  def save_editors
    unless self.editors_json.blank?
      Gwcircular::Role.destroy_all("title_id=#{self.id} and role_code = 'w'")
  ...
  def save_readers
    unless self.readers_json.blank?
      Gwcircular::Role.destroy_all("title_id=#{self.id} and role_code = 'r'")
  ...
  def save_readers_add
    unless self.editors_json.blank?
      ...
      item = Gwcircular::Role.find(:all, :conditions => "title_id=#{self.id} and role_code = 'r' and group_code = '0'")
      if item.length == 0
        ...
        groups.each do |group|
          unless group[1].blank?
            ...
            item_grp = Gwcircular::Role.find(:all, :conditions => "title_id=#{self.id} and role_code = 'r' and group_id = #{group[1]}")
