class Gwboard::Group < System::Group
  ...
  def self.level
    list = []
    System::Group.where(:state => 'enabled', :level_no => 2).order(:code).each do |dep|
      list << ['+' + dep.code + dep.name, 0]
      System::Group.where(:state => 'enabled', :parent_id => dep.id).order(:code).each do |sec|
  ...
  def self.level3(p_id)
    ...
    if p_id == 0
      ...
      System::Group.where(:state => 'enabled', :id => p_id).order(:code).each do |dep|
        list << [dep.name, dep.id]
      end
      System::Group.where(:state => 'enabled', :parent_id => p_id, :level_no => 3).order(:code).each do |dep|
        list << [dep.name, dep.id]
      end
    end
    return list
...
#    end
    ...
    return System::Group.where(group_cond).order('sort_no, code')
  end
...
#    end
    ...
    group_cond    << " and start_at <= '#{current_time.strftime("%Y-%m-%d 00:00:00")}'"
    group_cond    << " and (end_at IS Null or end_at = '0000-00-00 00:00:00' or end_at > '#{current_time.strftime("%Y-%m-%d 23:59:59")}' ) "
    return System::Group.where(group_cond).select('code, name').order('sort_no, code').to_a.index_by(&:code)
  end
  ...
  def self.level3_only(p_id)
    list = []
    if p_id == 0
      System::Group.where(:state => 'enabled', :level_no => 3).order(:code).each do |dep|
        list << [dep.name, dep.id]
      end
    else
      System::Group.where(:state => 'enabled', :id => p_id).order(:code).each do |dep|
