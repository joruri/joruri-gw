class Gwsub::Sb06BudgetAssignMain < Gwsub::GwsubPref
...
#  def before_save_setting_columns
  ...
  def self.main_state(all=nil)
    states = Gw.yaml_to_array_for_select 'gwsub_sb06_budget_assigns_main'
    states = [['すべて','0']] + states if all=='all'
    return states
  end
  def self.main_state_show(state)
    states = Gw.yaml_to_array_for_select 'gwsub_sb06_budget_assigns_main'
    state_show = []
