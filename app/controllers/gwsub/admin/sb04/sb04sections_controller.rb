class Gwsub::Admin::Sb04::Sb04sectionsController < Gw::Controller::Admin::Base
  ...
  def index2
    ...
    if @fyed_id.to_i==0
      ...
      fyear = Gwsub::Sb04EditableDate.order(fy_order).first
      if fyear.blank?
    ...
    assigns       = Gwsub::Sb04assignedjob.where(assign_cond).order(assign_order).group(assign_group)
    if assigns.blank?
      @items  = []
    else
      section_id = []
      ...
      sect_order  = "code"
      @items  = Gwsub::Sb04section.where(sect_cond).order(sect_order)
    end
  end
  ...
  def csvput
    ...
    csv = Gwsub::Sb04section.all.to_csv
    send_data @item.encode(csv), filename: "sb04sections_#{@item.encoding}.csv"
  end
...
#    return error_auth unless @u_role == true
    ...
    assign_order  = "section_id"
    assign_group  = "section_id"
    assigns       = Gwsub::Sb04CheckAssignedjob.where(assign_cond).order(assign_order).group(assign_group)
    if assigns.blank?
      @items  = []
    else
      section_id = []
      ...
      sect_cond   << " and id not in (#{section_ids})" unless section_ids.blank?
      sect_order  = "code"
      @items  = Gwsub::Sb04CheckSection.where(sect_cond).order(sect_order)
    end
  end
...
#    return error_auth unless @u_role == true
    ...
    @item = Gwsub::Sb04CheckSection.where(:id => params[:id]).first
    return http_error(404) if @item.blank?
  end
