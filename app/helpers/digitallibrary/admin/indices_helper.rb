# -*- encoding: utf-8 -*-
module Digitallibrary::Admin::IndicesHelper

  def digitallib_params_set
    ret =""
  ...
  def digitallib_form_indices
    ret = ""
    case params[:state]
    when "TODAY"
      ret = digitallib_form_indices_date
    when "DATE"
      ret = digitallib_form_indices_date
    when "GROUP"
      ret = digitallib_form_indices_group
    when "CATEGORY"
      ret = digitallib_form_indices_category
    else
      ret = digitallib_form_indices_date
    end
  ...
  def digitallib_form_indices_date
    brk_key = nil
    l_line = digitallib_start_line
    ...
    for item in @items
      ...
      ret += '<td style="text-align: left;">' + link_to(hbr(item.title), item.show_path(params[:state]) + "&pp=#{l_line}" + digitallib_params_set) + "</td>\n"
  ...
  def digitallib_form_indices_group
    brk_key = nil
    l_line = digitallib_start_line
    ret = '<table class="docformTitle">' + "\n"
    for item in @items
      ...
      ret += '<td ' + retstr_attache(item.attachmentfile) + 'style="text-align: center;">' + "</td>\n"
      ret += '<td ' + retstr_balloon(item.one_line_note)  + 'style="text-align: center;">' + "</td>\n"
      ret += '<td style="text-align: left;">' + link_to(hbr(item.title), item.show_path(params[:state]) + "&pp=#{l_line}&grp=#{params[:grp]}" + digitallib_params_set) + "</td>\n"
  ...
  def digitallib_form_indices_category
    brk_key = nil
    l_line = digitallib_start_line
    ret = '<table class="docformTitle">' + "\n"
    for item in @items
  ...
  def digitallib_admin_form_indices_date
    l_line = digitallib_start_line
    ret = '<table class="docformTitle">' + "\n"
    ret += "<tr>\n"
    ...
    for item in @items
      l_line = l_line + 1
      ret += "<tr>\n"
      ret += '<td style="text-align: center;">' + link_to(item.ststus_name, item.show_path(params[:state]) + "&pp=#{l_line}" + digitallib_params_set) + "</td>\n"
      ret += '<td style="text-align: center;">' + item.latest_updated_at.strftime('%Y-%m-%d %H:%M').to_s + "</td>\n"
      ret += '<td style="text-align: left;">' + link_to(hbr(item.title), item.show_path(params[:state]) + "&pp=#{l_line}" + digitallib_params_set) + "</td>\n"
      ret += '<td style="text-align: left;">' + if item.editordivision.blank? then item.createrdivision.to_s else item.editordivision.to_s end + "</td>\n"
      ret += "</tr>\n"
