    ...
    for @title in items
      ...
      for file in files
        ...
        unless doc.blank?
          ...
          aft_file_name = "/_admin/attaches/gwfaq/#{sprintf('%06d',file.title_id)}/#{parent_name}/#{str}/#{URI.encode(file.filename)}"
          sql = %Q[UPDATE `gwfaq_docs` SET body=REPLACE(body,'#{bef_file_name}', '#{aft_file_name}') WHERE id=#{file.parent_id};]
