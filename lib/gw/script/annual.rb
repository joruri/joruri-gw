...
class Gw::Script::Annual
  ...
  def self.gw_prop_other_renew
    ...
    r_groups.each do |r_group|
      ...
      _ids = Gw::PropOther.where(:gid => r_group.present_group_id).select("id")
      unless _ids.empty?
        _ids.each do |_id|
          ids << _id.id
        end
        ids = ids.uniq
        Gw::PropOther.where(:gid => r_group.present_group_id).update_all(:gid => r_group.incoming_group_id, :gname => r_group.incoming_group_name)
      ...
      _ids = Gw::PropOtherRole.where(:gid =>r_group.present_group_id ).select("id")
      unless _ids.empty?
        _ids.each do |_id|
          ids << _id.id
        end
        ids = ids.uniq
        Gw::PropOtherRole.where(:gid =>r_group.present_group_id ).update_all(:gid => r_group.incoming_group_id)
