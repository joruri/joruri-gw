namespace :git do
  ...
  task :pull do
    on roles(fetch(:web_role)), fetch(:run_options) do
      execute "/bin/bash -l -c 'cd #{fetch(:deploy_to)}; git pull --ff-only'"
    end
  end
  ...
  task :clone do
    on roles(fetch(:web_role)), fetch(:run_options) do
      execute "/bin/bash -l -c 'cd #{fetch(:deploy_to)}; git clone #{fetch(:repo_url)}'"
    end
  end
  ...
  task :fetch do
    on roles(fetch(:web_role)), fetch(:run_options) do
      execute "/bin/bash -l -c 'cd #{fetch(:deploy_to)}; git fetch'"
    end
  end
