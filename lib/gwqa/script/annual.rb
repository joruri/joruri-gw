class Gwqa::Script::Annual
  ...
  def self.renew_adms
    ...
    for adm in adms
      ...
      Gwqa::Adm.where(:group_id=>adm.group_id, :group_code=>adm.group_code).update_all(:group_id =>group.incoming_group_id, :group_code=>group.incoming_group_code, :group_name=>group.incoming_group_name )
  ...
  def self.renew_admingrps_json
    ...
    for title in titles
      ...
      if is_update
        ...
        Gwqa::Control.where(:id=>title.id).update_all(:admingrps_json=>JSON.generate(groups))
  ...
  def self.renew_roles
    ...
    for role in roles
      ...
      Gwqa::Role.where(:group_id=>role.group_id, :group_code=>role.group_code).update_all(:group_id=>group.incoming_group_id,:group_code=>group.incoming_group_code,:group_name=>group.incoming_group_name)
  ...
  def self.renew_editors_json
    ...
    for title in titles
      ...
      if is_update
        ...
        Gwqa::Control.where(:id=>title.id).update_all(:editors_json=>JSON.generate(groups))
        p "#{title.id}, #{update_field}, #{Time.now}."
      end
    end
  ...
  def self.renew_readers_json
    ...
    for title in titles
      ...
      if is_update
        ...
        Gwqa::Control.where(:id=>title.id).update_all(:readers_json=>JSON.generate(groups))
        p "#{title.id}, #{update_field}, #{Time.now}."
      end
    end
  ...
  def self.renew_docs_section_code
    ...
    items = Gwqa::Control.all.order('id')
    for @title in items
      begin
        doc_item = Gwqa::Doc
        ...
        for doc in docs
          ...
          Gwqa::Doc.where(:title_id => @title.id,:section_code=>doc.section_code).update_all(:section_code=>group.incoming_group_code,:section_name=>%Q(#{group.incoming_group_code}#{group.incoming_group_name}))
  ...
  def self.renew_dsp_admin_name
    ...
    for title in titles
      ...
      Gwqa::Control.where(:id=>title.id).update_all(:dsp_admin_name=>group.incoming_group_name)
