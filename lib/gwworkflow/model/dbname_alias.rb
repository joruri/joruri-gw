#encoding:utf-8
module Gwworkflow::Model::DbnameAlias

  def gwworkflow_select_status(params)
    raise "このメソッドは使いません！"
    str = ''
    case params[:cond]
    when "admin"
  ...
  def admin_flags(title_id)
    @is_sysadm = true if System::Model::Role.get(1, Site.user.id ,'gwworkflow', 'admin')
    @is_sysadm = true if System::Model::Role.get(2, Site.user_group.id ,'gwworkflow', 'admin') unless @is_sysadm
    @is_bbsadm = true if @is_sysadm

    unless @is_bbsadm
      item = Gwworkflow::Adm.new
      item.and :user_id, 0
  ...
  def get_writable_flag
    ...
    unless @is_writable
      ...
      items = Gwworkflow::Role.find(:all, :order=>'group_code', :conditions => sql.where)
      items.each do |item|
  ...
  def copy_attached_files(item, clone)
    ...
    total = Gwworkflow::File.sum(:size,:conditions => 'unid = 1')
    total = 0 if total.blank?
    @title.upload_graphic_file_size_currently = total.to_f
    total = Gwworkflow::File.sum(:size,:conditions => 'unid = 2')
    total = 0 if total.blank?
